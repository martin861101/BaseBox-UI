...
export function AppSidebar() {
  ...
  return (
    <Sidebar collapsible="icon">
      <SidebarHeader className="py-4 border-b border-border/40">
        <div className="flex items-center justify-center h-40">
          <img src={logo} alt="Mad Scientist" className="h-40 w-40 object-contain" />
        </div>
      </SidebarHeader>
      <SidebarContent>
        <SidebarGroup>
              ...
              <SidebarMenu>
                <TooltipProvider delayDuration={300}>
                  {servers.map((s) => {
                    ...
                    const ok = h?.ok;
                    const dot =
                      h == null ? "bg-muted-foreground/40" : ok ? "bg-success" : "bg-destructive";
                    return (
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <SidebarMenuButton asChild>
                              <Link to="/monitoring">
                                <span
                                  className={`h-2 w-2 shrink-0 rounded-full ${dot} ${ok ? "animate-pulse" : ""}`}
                                />
                          ...
                          <TooltipContent side="right">
                            <div className="text-xs">
                              ...
                              <div className="mt-1">
                                {ok ? (
                                  <>
                                    Online · {h?.latencyMs}ms · CPU {h?.cpu?.toFixed(0) ?? "?"}%
                                  </>
                                ) : (
