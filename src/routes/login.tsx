...
function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After successful login, navigate to monitoring
      nav({ to: "/monitoring" });
    } catch (e: any) {
      setErr(e.message);
    } finally {
        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-4">
            ...
            <p className="rounded-md border border-border/60 bg-muted/40 p-2 text-xs text-muted-foreground">
              ...
              <code className="font-mono text-foreground">admin</code> to explore the UI without the
              agent.
            </p>
